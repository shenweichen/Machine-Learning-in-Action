...
def splitDataSet(dataSet, axis, value):#按照给定特征划分数据集
    ...
    for featVec in dataSet:
        if featVec[axis] == value:
            reducedFeatVec = featVec[:axis]     #抽取其他特征
            reducedFeatVec.extend(featVec[axis+1:])
            retDataSet.append(reducedFeatVec) #添加该实例
    return retDataSet
...
def createTree(dataSet,labels):#创建树的函数代码
    ...
    if classList.count(classList[0]) == len(classList): #若第一个类标签的个数等于类标签的总数
        return classList[0]#所有的类标签完全相同，直接返回类标签
...
def classify(inputTree,featLabels,testVec):#使用决策树的分类函数
    ...
    featIndex = featLabels.index(firstStr)#查找当前列表第一个匹配firstStr的元素
